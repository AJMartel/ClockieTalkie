...
volatile byte say_time_position = 0;

// simple switch cae for sayign number
void say_number(byte number) {
  switch (number) {
    case 1:
      voice.say(spONE);
      break;
...
boolean say_time_by_position() {
  boolean isOver = false;
  switch (say_time_position) {
    case 0:
      // talk
      voice.say(spGOOD);
      break;
    ...
    case 6: {
        // HOURS
        byte tempHours = Hours;
        if (tempHours == 0)  // midnight
          tempHours = 12;
        say_number(tempHours);
      } //end case 
      break;
    case 7: {
        // minutes
        if (Minutes < 10) {
          ...
          if (Minutes - (tensOf * 10) > 0)
            say_number(Minutes - tensOf * 10);
        } //end if
      } //end case
      break;
    case 8:
    ...
    case 9:
      //short pause
      voice.say(spPAUSE1);
      isOver = true; // set flag
      break;
  } //end switch

  // if over
  if (isOver)
    say_time_position = 0; // reset the position
  else
    say_time_position++; // move to next position
